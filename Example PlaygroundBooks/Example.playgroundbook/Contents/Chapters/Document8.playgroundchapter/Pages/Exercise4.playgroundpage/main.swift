...
To solve this puzzle, you’ll write conditional code that compares the value of a gem-counting variable with `switchCounter`, a constant that stores the number of switches that randomly appear in the puzzle. To compare these two values, use a [comparison operator](glossary://comparison%20operator) such as `<`, `>`, `==`, or `!=`.
