...
public func assessmentPoint() -> AssessmentResults {
    ...
    if checker.didUseConditionalStatement && runCount > 3 {
        ...
        success = NSLocalizedString("### Use conditional code. \nAlthough you solved the puzzle this time, if you rerun your code, it probably won’t work again. \nTry using conditional code that will work in many different situations.\n", comment:"Success message")
    }
    ...
    if containsNotOperator && checker.didUseConditionalStatement {
        success = NSLocalizedString("### You’re amazing! \nUsing the [logical NOT operator](glossary://logical%20NOT%20operator), you reversed a [Boolean](glossary://Boolean) value, allowing you to check the opposite of a condition like `isOnGem` or `isOnClosedSwitch`. What you’re saying is, “If NOT on a gem, do this.” \n\n[**Next Page**](@next)", comment:"Success message")
        
        
    } else if !containsNotOperator && checker.didUseConditionalStatement {
        hints.insert(NSLocalizedString("Make sure your condition looks like this, with no spaces between the ! and `isOnGem`: \n\n`!isOnGem`.", comment:"Hint"), at: 0)
    }
