...
public func assessmentPoint() -> AssessmentResults {
    ...
    if !checker.didUseConditionalStatement("if") {
        hints[0] = NSLocalizedString("You need to use an `if` statement to determine whether to toggle the switch. Select `if` in the shortcut bar to insert an if statement into your code. Then use the condition `isOnClosedSwitch` to determine whether to toggle the switch.", comment:"Hint")
