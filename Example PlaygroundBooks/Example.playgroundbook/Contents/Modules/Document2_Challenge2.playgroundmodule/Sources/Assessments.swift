...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        NSLocalizedString("Try to find the repeating patterns in the puzzle. For example, each row contains three gems in a line.", comment:"Hint"),
        ...
        NSLocalizedString("This puzzle is a **challenge**. Challenges give you an opportunity to test your coding skills. Because challenges may be solved in different ways, a solution isn’t always provided for you. Instead, your goal is to think of your own approach.", comment:"Hint")
        
        ]
    let customFunction = checker.customFunctions.first ?? ""
    let allFunctions = checker.customFunctions
    ...
    if checker.didUseForLoop || checker.didUseWhileLoop {
        ...
        hints[0] = NSLocalizedString("Oops, you called `collectGem()` when no gem was present. This is a bug in your code—you should collect a gem only if one is present on the tile.", comment:"Hint")
    } else if checker.functionCallCount(forName: customFunction) == 0 && allFunctions.count <= 1 {
        ...
        hints[0] = NSLocalizedString("Define your function by giving it a name and a set of commands. Then be sure to [call](glossary://call) your function by selecting the function name in the shortcut bar.", comment:"Hint")
