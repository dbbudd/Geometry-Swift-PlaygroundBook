...
struct NodeFactory {
    ...
    static func make(from message: PlaygroundValue, within world: GridWorld) throws -> Item? {
        ...
        if let possibleItem = world.item(forID: info.id),
            ...
            guard let newItem = try make(with: info.identifier, in: world, from: info.stateInfo) else {
    ...
    static func make(with identifier: WorldNodeIdentifier, in world: GridWorld, from stateInfo: [PlaygroundValue]) throws -> Item? {
        let node: Item
        ...
        case .portal:
            guard stateInfo.count >= 3,
                ...
                let color = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(colorData) as? _Color
                else { return nil }
        ...
        case .randomNode:
            guard case let .string(id)? = stateInfo.first,
                let identifier = WorldNodeIdentifier(rawValue: id),
                let resemblingItem = try make(with: identifier, in: world, from: []) else { return nil }
