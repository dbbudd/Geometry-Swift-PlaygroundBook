...
final class CommandDecoder {
    ...
    func command(from message: Message) throws -> Command? {
        guard let action = try action(from: message),
    ...
    func action(from message: Message) throws -> Action? {
        ...
        case .add:
            var indices = [Int]()
            for message in args {
                guard let item = try NodeFactory.make(from: message, within: world) else {
...
extension Color: MessageConstructible {
    var message: PlaygroundValue {
        do {
            let valueData = try NSKeyedArchiver.archivedData(withRootObject: rawValue, requiringSecureCoding: false)
            return .data(valueData)
        }
        catch {
            fatalError("*** There was a problem archiving a Color value: \(error.localizedDescription)")
        }
    }
    ...
    convenience init?(message: PlaygroundValue) {
        ...
        do {
            guard let color = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? _Color else { return nil }
            self.init(color)
        }
        catch {
            fatalError("*** There was a problem unarchiving a Color value: \(error.localizedDescription)")
        }
    }
}
