...
class CoordinateMarker: SCNNode {
    ...
    init(coordinate: Coordinate) {
        ...
        let minVec = labelGeo.boundingBox.min
        let maxVec = labelGeo.boundingBox.max
        let bound = SCNVector3Make(maxVec.x - minVec.x,
                                   maxVec.y - minVec.y,
                                   maxVec.z - minVec.z)
        let plane = SCNPlane(width: CGFloat(bound.x + 5),
                            height: CGFloat(bound.y + 5))
        plane.cornerRadius = 20.0
        plane.firstMaterial?.diffuse.contents = UIColor.systemBackground.withAlphaComponent(0.65)
