...
public final class Gem: Item, LocationConstructible, NodeConstructible {
    ...
    private lazy var collectSoundAction: SCNAction = {
        return .playSoundEffect(Gem.collectSound)
    }()
    ...
    public func updateAppearance(traitCollection: UITraitCollection) {
        ...
        if let mat = scnNode.childNode(withName: "zon_item_gem_a_GEO_0001", recursively: true)?.firstGeometry?.firstMaterial {
            mat.diffuse.contents = UIColor(named: "gem_diffuse_color")!
            mat.emission.contents = UIColor(named: "gem_emission_color")!
        }
    }
