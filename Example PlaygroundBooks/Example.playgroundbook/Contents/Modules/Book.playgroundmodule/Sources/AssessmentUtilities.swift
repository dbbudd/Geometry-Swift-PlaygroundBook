...
public func updateAssessmentForSandboxWorld(successMessage: String, failureHints: [String], solution: String?) -> AssessmentResults {
    if let passed = assessmentInfo?.passedCriteria, passed {
        let key = "success"
        if case let .integer(runs)? = PlaygroundKeyValueStore.current[key] {
            if runs == 0 {
                PlaygroundKeyValueStore.current[key] = .integer(1)
                return .pass(message: successMessage)
            }
            else {
...
extension GridWorld {
    ...
    public func coveredInBlocks() -> Bool {
        // SD of 20 for generating a landscape filled with blocks of alternating heights of 0 and 1.
        // SD of 80 for generating a landscape filled with blocks of alternating heights of 0 and 3.
        // We expect a SD of at least 50 for this assessment.
        return blockStandardDeviation() > 50
    }
    ...
    public func buildAnIsland() -> Bool {
        ...
        for coor in perimeter {
            ...
            if existingBlocks(at: [coor]).count > 0 {
                return false
            }
        }
    ...
    public func didChangePageText(differenceCount count: Int = 10) -> Bool {
        ...
        let pageTextCount = trimmedText.count
