...
public class GridWorld {
    ...
    var spokeAboutRandomNodes = false
    ...
    @discardableResult
    func findComponent<T : Performer>(ofType componentType: T.Type) -> Int? {
        ...
        case .add(let indices):
            for item in items(from: indices) {
                if item.identifier == .block {
                    if !spokeAboutRandomNodes {
                        spokeAboutRandomNodes = true
                        text.append(NSLocalizedString("Random blocks are being placed in the world.", comment: "AX announcement of random blocks being placed"))
                    }
                } else {
...
extension GridWorld {
    ...
    public func existingItems<Child: Item>(ofType itemType: Child.Type, at coordinates: [Coordinate]) -> [Child] {
        return coordinates.flatMap { coordinate -> [Child] in
            ...
            let nodes = candidateChildren.compactMap { node -> Child? in
    ...
    public func removeItems(at coordinates: [Coordinate]) {
        removeItems(at: coordinates, ignoringBlocks: true)
    }
    
    public func removeItems(at coordinates: [Coordinate], ignoringBlocks: Bool) {
        for coordinate in coordinates {
    ...
    func hitNodes(containingNames nodeNames: [String], at coordinate: Coordinate) -> [SCNNode] {
        ...
        return rootNode.hitTestWithSegment(from: abovePosition, to: belowPosition).compactMap { hit -> SCNNode? in
            let node = hit.node
            let name = node.name ?? ""
