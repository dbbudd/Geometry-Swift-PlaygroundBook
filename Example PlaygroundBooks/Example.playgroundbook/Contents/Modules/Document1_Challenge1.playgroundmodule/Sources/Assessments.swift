...
public func assessmentPoint() -> AssessmentResults {
    ...
    if let openSwitchIndex = queue.indexOfFirstCorrectToggle(), let collectGemIndex = queue.indexOfFirstCollectedGem() {
        if openSwitchIndex > collectGemIndex {
            success = NSLocalizedString("You solved the puzzle, but you used your commands in the wrong order. First toggle the switch, then move through the portal and collect the gem.", comment: "Success message")
        }
    }
